...
var _toConsumableArray = _interopDefault(
  ...
  _possibleConstructorReturn = _interopDefault(
    require('@babel/runtime/helpers/possibleConstructorReturn')
  ),
  _getPrototypeOf = _interopDefault(
  ...
  useStyles = require('substyle'),
  useStyles__default = _interopDefault(useStyles),
  ...
  findPositionOfCapturingGroup = function(markup, parameterName) {
    invariant(
      'id' === parameterName || 'display' === parameterName,
      'Second arg must be either "id" or "display", got: "'.concat(
        parameterName,
        '"'
      )
    )
    var indexDisplay = markup.indexOf(PLACEHOLDERS.display),
      indexId = markup.indexOf(PLACEHOLDERS.id)
    return (
      indexDisplay < 0 && (indexDisplay = null),
      indexId < 0 && (indexId = null),
      invariant(
        null !== indexDisplay || null !== indexId,
        "The markup '".concat(
          markup,
      ...
      null !== indexDisplay && null !== indexId
        ? ('id' === parameterName && indexId <= indexDisplay) ||
          ('display' === parameterName && indexDisplay <= indexId)
          ? 0
          : 1
        : 0
    )
  ...
  combineRegExps = function(regExps) {
    ...
    return new RegExp(
      regExps
        .map(function(regex) {
          var _serializedRegexParse = serializedRegexParser.exec(
              regex.toString()
            ),
  ...
  countPlaceholders = function(markup) {
    var count = 0
    return (
      markup.indexOf('__id__') >= 0 && count++,
      markup.indexOf('__display__') >= 0 && count++,
      count
    )
  },
  emptyFn = function() {},
  iterateMentionsMarkup = function(value, config, markupIteratee) {
    for (
      var match,
        textIteratee =
          arguments.length > 3 && void 0 !== arguments[3]
            ? arguments[3]
            : emptyFn,
        regex = combineRegExps(
          config.map(function(c) {
            return c.regex
          })
        ),
        accOffset = 2,
        captureGroupOffsets = config.map(function(_ref) {
          var result,
            markup = _ref.markup
          return void 0 !== markup
            ? ((result = accOffset),
        ...
        start = 0,
        currentPlainTextIndex = 0;
      null !== (match = regex.exec(value));

    ) {
      var offset = captureGroupOffsets.find(function(o) {
          return !!match[o]
        }),
        mentionChildIndex = captureGroupOffsets.indexOf(offset),
        _config$mentionChildI = config[mentionChildIndex],
        markup = _config$mentionChildI.markup,
        displayTransform = _config$mentionChildI.displayTransform,
        idPos = offset + findPositionOfCapturingGroup(markup, 'id'),
        displayPos = offset + findPositionOfCapturingGroup(markup, 'display'),
        id = match[idPos],
        display = displayTransform(id, match[displayPos]),
  ...
  getPlainText = function(value, config) {
    ...
    return (
      iterateMentionsMarkup(
        value,
        config,
        function(match, index, plainTextIndex, id, display) {
          result += display
        },
        function(plainText) {
          result += plainText
  ...
  mapPlainTextIndex = function(value, config, indexInPlainText) {
    var result,
      inMarkupCorrection =
        arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : 'START'
      ...
      iterateMentionsMarkup(
        ...
        function(
          ...
          void 0 === result &&
            ...
            (result =
              'NULL' === inMarkupCorrection
                ? null
                : index + ('END' === inMarkupCorrection ? markup.length : 0))
        },
        function(substr, index, substrPlainTextIndex) {
          void 0 === result &&
            substrPlainTextIndex + substr.length >= indexInPlainText &&
  ...
  applyChangeToValue = function(value, plainTextValue, _ref, config) {
    var selectionStartBefore = _ref.selectionStartBefore,
      ...
      oldPlainTextValue = getPlainText(value, config),
      lengthDelta = oldPlainTextValue.length - plainTextValue.length
    null === selectionStartBefore &&
      (selectionStartBefore = selectionEndAfter + lengthDelta),
      null === selectionEndBefore &&
    ...
    var insert = plainTextValue.slice(selectionStartBefore, selectionEndAfter),
      spliceStart = Math.min(selectionStartBefore, selectionEndAfter),
      spliceEnd = selectionEndBefore
    ...
    var mappedSpliceStart = mapPlainTextIndex(
      ...
      controlSpliceStart = mapPlainTextIndex(
        value,
        config,
        spliceStart,
        'NULL'
      ),
      controlSpliceEnd = mapPlainTextIndex(value, config, spliceEnd, 'NULL'),
      willRemoveMention =
        null === controlSpliceStart || null === controlSpliceEnd,
  ...
  findStartOfMentionInPlainText = function(value, config, indexInPlainText) {
    var result = indexInPlainText,
      foundMention = !1
    if (
      (iterateMentionsMarkup(value, config, function(
        markup,
        index,
        mentionPlainTextIndex,
        id,
        ...
        mentionPlainTextIndex <= indexInPlainText &&
          mentionPlainTextIndex + display.length > indexInPlainText &&
          ((result = mentionPlainTextIndex), (foundMention = !0))
      }),
      foundMention)
    )
      return result
    ...
    return new RegExp(
      escapedMarkup
        .replace(
          ...
          '([^'.concat(escapeRegex(charAfterDisplay || ''), ']+?)')
        )
        .replace(
          PLACEHOLDERS.id,
          '([^'.concat(escapeRegex(charAfterId || ''), ']+?)')
        )
    )
  },
  ...
  getSubstringIndex = function(str, substr, ignoreAccents) {
    return ignoreAccents
      ? normalizeString(str).indexOf(normalizeString(substr))
...
function createDefaultStyle(defaultStyle, getModifiers) {
  return function(ComponentToWrap) {
    var DefaultStyleEnhancer = React__default.forwardRef(function(
        _ref,
        forwardedRef
      ) {
        var style = _ref.style,
          ...
          modifiers = getModifiers ? getModifiers(rest) : void 0,
          styles = useStyles__default(
            defaultStyle,
            {
              style: style,
              className: className,
    ...
    return (
      (DefaultStyleEnhancer.displayName = 'defaultStyle('.concat(
        displayName,
        ')'
      )),
...
var _generateComponentKey = function(usedKeys, id) {
  return (
    usedKeys.hasOwnProperty(id) ? usedKeys[id]++ : (usedKeys[id] = 0),
    id + '_' + usedKeys[id]
  )
}
...
function Highlighter(_ref) {
  var selectionStart = _ref.selectionStart,
    ...
    style = (_ref.singleLine, _ref.style),
  ...
  var resultComponents = [],
    ...
    substringComponentKey = 0,
    renderSubstring = function(string, key) {
      return React__default.createElement(
        'span',
        _extends({}, style('substring'), {
          key: key,
        }),
        string
      )
    },
    getMentionComponentForMatch = function(
      id,
      display,
      mentionChildIndex,
      key
...
function Suggestion(_ref) {
  var display,
    ...
    renderHighlightedDisplay = function(display) {
      ...
      return -1 === i
        ? React__default.createElement('span', style('display'), display)
        : React__default.createElement(
            'span',
            style('display'),
  ...
  return React__default.createElement(
    ...
    ((display = getDisplay()),
    (highlightedDisplay = renderHighlightedDisplay(display)),
    renderSuggestion
...
function LoadingIndicator(_ref) {
  var style = _ref.style,
    className = _ref.className,
    classNames = _ref.classNames,
    styles = useStyles__default(defaultstyle, {
      style: style,
      className: className,
      classNames: classNames,
    }),
  ...
  return React__default.createElement(
    ...
    React__default.createElement(
      ...
      React__default.createElement(
        'div',
        spinnerStyles(['element', 'element1'])
      ),
      React__default.createElement(
        'div',
        spinnerStyles(['element', 'element2'])
      ),
      React__default.createElement(
        'div',
        spinnerStyles(['element', 'element3'])
      ),
      React__default.createElement(
        'div',
        spinnerStyles(['element', 'element4'])
      ),
      React__default.createElement(
        'div',
        spinnerStyles(['element', 'element5'])
      )
    )
  )
}
...
function SuggestionsOverlay(_ref) {
  var id = _ref.id,
    ...
    suggestions = void 0 === _ref$suggestions ? {} : _ref$suggestions,
    ...
    function() {
      if (
        ...
        var scrollTop = ulElement.scrollTop,
          _ulElement$children$f = ulElement.children[
            focusIndex
          ].getBoundingClientRect(),
          top = _ulElement$children$f.top,
          bottom = _ulElement$children$f.bottom,
          topContainer = ulElement.getBoundingClientRect().top
        ;(bottom = bottom - topContainer + scrollTop),
          (top = top - topContainer + scrollTop) < scrollTop
            ? (ulElement.scrollTop = top)
            : bottom > ulElement.offsetHeight &&
  ...
  var suggestionsToRender,
    renderSuggestion = function(result, queryInfo, index) {
      var isFocused = index === focusIndex,
        childIndex = queryInfo.childIndex,
        query = queryInfo.query,
        renderSuggestion = React.Children.toArray(children)[childIndex].props
          .renderSuggestion
  ...
  return isOpened
    ? React__default.createElement(
        ...
        React__default.createElement(
          ...
          ((suggestionsToRender = Object.values(suggestions).reduce(function(
            accResults,
            _ref2
          ) {
...
var makeTriggerRegex = function(trigger) {
    ...
    var allowSpaceInQuery = options.allowSpaceInQuery,
      escapedTriggerChar = escapeRegex(trigger)
    return new RegExp(
      '(?:^|\\s)('
        .concat(escapedTriggerChar, '([^')
        .concat(allowSpaceInQuery ? '' : '\\s')
        .concat(escapedTriggerChar, ']*))$')
    )
  },
    ...
    return data instanceof Array
      ? function(query, callback) {
          for (var results = [], i = 0, l = data.length; i < l; ++i) {
            ...
            getSubstringIndex(display, query, ignoreAccents) >= 0 &&
              results.push(data[i])
          }
          return results
        }
  ...
  MentionsInput = (function(_React$Component) {
    ...
    function MentionsInput(_props) {
      ...
      return (
            ...
            return _objectSpread(
              ...
              _this.isOpened() && {
                ...
                'aria-activedescendant': getSuggestionHtmlId(
                  _this.uuidSuggestionsOverlay,
                  _this.state.focusIndex
                ),
              }
            )
          }
        ...
        _defineProperty(
          ...
          function() {
            ...
            return React__default.createElement(
              'div',
              style('control'),
              _this.renderHighlighter(),
              renderInput
                ? _this.renderCustom(inputProps)
                : singleLine
                ? _this.renderInput(inputProps)
                : _this.renderTextarea(inputProps)
            )
          }
        ),
        _defineProperty(_assertThisInitialized(_this), 'renderCustom', function(
          props
        ) {
        ...
        _defineProperty(_assertThisInitialized(_this), 'setInputRef', function(
          el
        ) {
          _this.inputElement = el
          var inputRef = _this.props.inputRef
          'function' === typeof inputRef
            ? inputRef(el)
        ...
        _defineProperty(
          ...
          function() {
            ...
            var _this$state$suggestio = _this.state.suggestionsPosition,
              ...
              suggestionsNode = React__default.createElement(
                SuggestionsOverlay$1,
                {
                  ...
                  customSuggestionsContainer:
                    _this.props.customSuggestionsContainer,
                  ...
                  a11ySuggestionsListLabel:
                    _this.props.a11ySuggestionsListLabel,
                },
                _this.props.children
              )
            return _this.props.suggestionsPortalHost
              ? ReactDOM.createPortal(
                  suggestionsNode,
                  _this.props.suggestionsPortalHost
                )
              : suggestionsNode
          }
        ),
        ...
        _defineProperty(
          ...
          function() {
            return getPlainText(
              _this.props.value || '',
              readConfigFromChildren(_this.props.children)
            )
          }
        ),
        _defineProperty(
          ...
          function(event) {
            ...
            return _this.props.onChange
              ? (_this$props4 = _this.props).onChange.apply(
                  _this$props4,
                  [event].concat(args)
                )
              : _this.props.valueLink
              ? (_this$props$valueLink =
                  _this.props.valueLink).requestChange.apply(
                  _this$props$valueLink,
                  [event.target.value].concat(args)
                )
              : void 0
          }
        ),
        _defineProperty(_assertThisInitialized(_this), 'handleChange', function(
          ...
          var value = _this.props.value || '',
            config = readConfigFromChildren(_this.props.children),
          ...
          var selectionStart = ev.target.selectionStart,
            ...
            startOfMention = findStartOfMentionInPlainText(
              value,
              config,
              selectionStart
            )
          void 0 !== startOfMention &&
            _this.state.selectionEnd > startOfMention &&
            ((selectionEnd = selectionStart =
              startOfMention +
        ...
        _defineProperty(
          ...
          function(ev) {
            if (
              0 !== countSuggestions(_this.state.suggestions) &&
              _this.suggestionsElement
            )
              switch (
                (Object.values(KEY).indexOf(ev.keyCode) >= 0 &&
        ...
        _defineProperty(_assertThisInitialized(_this), 'shiftFocus', function(
          delta
        ) {
          var suggestionsCount = countSuggestions(_this.state.suggestions)
          _this.setState({
            focusIndex:
              (suggestionsCount + _this.state.focusIndex + delta) %
              suggestionsCount,
        ...
        _defineProperty(
          ...
          function() {
            var _this$state2 = _this.state,
              ...
              _Object$values$reduce = Object.values(suggestions).reduce(
                function(acc, _ref) {
          ...
          function() {
            var caretPosition = _this.state.caretPosition,
              ...
              allowSuggestionsAboveCursor =
                _this$props5.allowSuggestionsAboveCursor,
            ...
            if (caretPosition && _this.suggestionsElement) {
              var suggestions = _this.suggestionsElement,
                highlighter = _this.highlighterElement,
                caretOffsetParentRect = highlighter.getBoundingClientRect(),
                caretHeight = getComputedStyleLengthProp(
                  highlighter,
                  'font-size'
                ),
                viewportRelative = {
                  ...
                  top:
                    caretOffsetParentRect.top + caretPosition.top + caretHeight,
                },
              ...
              if (suggestions) {
                ...
                if (suggestionsPortalHost) {
                  ...
                  var left = viewportRelative.left,
                    top = viewportRelative.top
                  ;(left -= getComputedStyleLengthProp(
                    suggestions,
                    'margin-left'
                  )),
                    (top -= getComputedStyleLengthProp(
                      suggestions,
                      'margin-top'
                    )),
                  ...
                  var _left = caretPosition.left - highlighter.scrollLeft,
                    _top = caretPosition.top - highlighter.scrollTop
                  _left + suggestions.offsetWidth >
                  _this.containerElement.offsetWidth
                    ? (position.right = 0)
                    : (position.left = _left),
                    (allowSuggestionsAboveCursor &&
                    ...
                    forceSuggestionsAboveCursor
                      ? (position.top =
                          _top - suggestions.offsetHeight - caretHeight)
                      : (position.top = _top)
                }
                ;(position.left === _this.state.suggestionsPosition.left &&
                  position.top === _this.state.suggestionsPosition.top &&
                  position.position ===
                    _this.state.suggestionsPosition.position) ||
                  _this.setState({
        ...
        _defineProperty(
          ...
          function(plainTextValue, caretPosition) {
            _this._queryId++,
              (_this.suggestions = {}),
              _this.setState({
                suggestions: {},
            ...
            var value = _this.props.value || '',
              ...
              config = readConfigFromChildren(children),
              positionInValue = mapPlainTextIndex(
                value,
                config,
                caretPosition,
                'NULL'
            ...
            if (null !== positionInValue) {
              var substringStartIndex = getEndOfLastMention(
                  value.substring(0, positionInValue),
                  config
                ),
                substring = plainTextValue.substring(
                  substringStartIndex,
                  caretPosition
                )
              React__default.Children.forEach(children, function(
                ...
                if (child) {
                  var regex = makeTriggerRegex(
                      child.props.trigger,
                      _this.props
                    ),
                    match = substring.match(regex)
                  if (match) {
                    var querySequenceStart =
                      substringStartIndex +
        ...
        _defineProperty(_assertThisInitialized(_this), 'queryData', function(
          query,
          childIndex,
          querySequenceStart,
          querySequenceEnd,
          ...
          var _this$props6 = _this.props,
            ...
            mentionChild = React.Children.toArray(children)[childIndex],
            syncResult = getDataProvider(
              mentionChild.props.data,
              ignoreAccents
            )(
              query,
              _this.updateSuggestions.bind(
                null,
                _this._queryId,
                childIndex,
                query,
          ...
          function(
            ...
            if (queryId === _this._queryId) {
              _this.suggestions = _objectSpread(
                _objectSpread({}, _this.suggestions),
                {},
                _defineProperty({}, childIndex, {
                  queryInfo: {
                    childIndex: childIndex,
                    query: query,
              ...
              var focusIndex = _this.state.focusIndex,
                suggestionsCount = countSuggestions(_this.suggestions)
              _this.setState({
        ...
        _defineProperty(_assertThisInitialized(_this), 'addMention', function(
          _ref2,
          _ref3
        ) {
          var id = _ref2.id,
            ...
            config = readConfigFromChildren(_this.props.children),
            _mentionsChild$props = React.Children.toArray(_this.props.children)[
              childIndex
            ].props,
            ...
            insert = makeMentionsMarkup(markup, id, display)
          ...
          var eventMock = {
            ...
            mentions = getMentions(newValue, config),
            newPlainTextValue = spliceString(
              plainTextValue,
              querySequenceStart,
              querySequenceEnd,
              displayValue
          ...
          _this.executeOnChange(
            ...
            onAdd && onAdd(id, display, start, end),
            _this.clearSuggestions()
        }),
        _defineProperty(_assertThisInitialized(_this), 'isLoading', function() {
          var isLoading = !1
          return (
            React__default.Children.forEach(_this.props.children, function(
              child
            ) {
        ...
        _defineProperty(_assertThisInitialized(_this), 'isOpened', function() {
          return (
            isNumber(_this.state.selectionStart) &&
            (0 !== countSuggestions(_this.state.suggestions) ||
              _this.isLoading())
          )
        }),
        ...
        (_this.suggestions = {}),
        (_this.uuidSuggestionsOverlay = Math.random()
          .toString(16)
          .substring(2)),
        (_this.handleCopy = _this.handleCopy.bind(
          _assertThisInitialized(_this)
        )),
      ...
      _createClass(MentionsInput, [
        ...
        },
        {
          ...
          value: function(prevProps, prevState) {
            prevState.suggestionsPosition === this.state.suggestionsPosition &&
              this.updateSuggestionsPosition(),
          ...
          value: function(event) {
            if (
              ...
              var _this$state3 = this.state,
                ...
                config = readConfigFromChildren(children),
                markupStartIndex = mapPlainTextIndex(
                  value,
                  config,
                  selectionStart,
                  'START'
                ),
                markupEndIndex = mapPlainTextIndex(
                  value,
                  config,
                  selectionEnd,
                  'END'
                ),
                pastedMentions = event.clipboardData.getData(
                  'text/react-mentions'
                ),
